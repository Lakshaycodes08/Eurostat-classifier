# Swytchcode Agent Contract (GitHub Copilot)
...
If VS Code MCP is configured, MCP tools are available:
...
7. swytchcode diff <library>: Show method-level signature changes in a pending upgrade proposal before approving
8. swytchcode discover "<intent>" [--library <name>]: Find API capabilities matching a natural language description
9. swytchcode plan <canonical_id>: Show the steps of a workflow before executing it
10. swytchcode doctor: Diagnose project setup
...
### Debugging execution:
- `swytchcode exec <canonical_id> --dry-run`: Preview the exact HTTP request (method, URL, headers, body) without making the call.
- `swytchcode exec <canonical_id> --verbose`: Log full request + response JSON to stderr (sensitive headers like `Authorization` are redacted). Redirect with `2>debug.log`.
...
Run `swytchcode list tooling` to see what is currently enabled in tooling.json.
...
- Inspect its input/output contract using `swytchcode info <canonical_id>`.
...
RULE: Before generating any execution code, verify the canonical_id exists in tooling.json
using `swytchcode list tooling`. If it is not listed, call `swytchcode add` first. Never
generate code for a tool that has not been added.
...
1. **List workflows** — `swytchcode list workflows` shows workflow canonical IDs and their integration (`project.library@version`).
...
Do not guess step IDs or I/O from workflow names. Always use `swytchcode list` and `swytchcode info` to discover workflow and step canonical IDs and their contracts.

Once you get the information about all the steps/methods, create integration code for the methods in order of the increasing index number. They should be different integration calls. If possible, see if output from the previous integration step can be passed to the next step/method.

---

## Keeping integrations up to date

The TinyFish agent continuously monitors your integrations for breaking changes and new versions.
...
Copilot is **not exploring** Swytchcode.

Copilot is **compiling against Swytchcode**.
