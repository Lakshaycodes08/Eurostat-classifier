// get.go extracts get command logic for MCP use.
package commands
...
// RunGet runs the get command logic.
	...
	for i, bundle := range bundlesResp.Bundles {
		...
		if _, err := os.Stat(versionedDir); err == nil {
			if _, err := os.Stat(wrekenPath); err == nil {
				if !yes {
					bundleSpinner.Stop()
					return fmt.Errorf("Version %q for %s/%s already exists; set yes parameter to true to overwrite", bundle.Version, projectName, bundle.Integration)
				}
			}
		}
