...
A: Only if the host is loopback: `localhost`, `127.0.0.1`, or `::1`. Otherwise the base URL must be `https://`. This is enforced for every exec (including in GitHub Actions, GitLab CI, and Docker). See the manifest section in [docs/config-spec.md](https://gitlab.com/swytchcode/swytchcode-cli/-/blob/main/docs/config-spec.md).
