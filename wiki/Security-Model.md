...
- **Integration base URLs** – Tool HTTP calls require **`https://`** unless the base URL is **`http://`** on loopback only (`localhost`, `127.0.0.1`, `::1`). See [Config spec → manifest](https://gitlab.com/swytchcode/swytchcode-cli/-/blob/main/docs/config-spec.md) (HTTPS and HTTP base URLs).
...
Consolidated doc in-repo: [docs/security.md](https://gitlab.com/swytchcode/swytchcode-cli/-/blob/main/docs/security.md). Run **`swytchcode doctor`** (or MCP **`swytchcode_doctor`**) for a quick local posture check.
